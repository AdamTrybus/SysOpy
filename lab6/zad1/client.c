...
void handle_list() {
    time_t msg_time = time(NULL);
    msg_buff msg = { .mtype = LIST, .client_id = client_idx, .time_struct = *localtime(&msg_time) };
    msgsnd(server_q, &msg, sizeof(msg), 0);
    msgrcv(client_qid, &msg, sizeof(msg), 0, 0);
